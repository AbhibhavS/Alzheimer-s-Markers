setwd("C:/Protein/AD Gene Expression/Abhibhav/Hippo")
set.seed(123)
...
gse3=getGEO(filename="GSE28146_series_matrix.txt.gz",
            GSEMatrix=TRUE,getGPL = TRUE)
GSE28146<- as.matrix(gse3)
GSE28146<-as.data.frame(GSE28146)
colnames(GSE28146)<- c(read.csv("sample info_GSE28146.csv",
                               header =T))[[1]]

GSE_GPL570<- cbind(GSE5281, GSE48350, GSE28146[,9:23])
colnames(GSE_GPL570)

GSE_HIP<-GSE_GPL570[,c(grep("Hippocampus", colnames(GSE_GPL570)),
                       grep("hippocampus", colnames(GSE_GPL570)))]
colnames(GSE_HIP)
##write.csv(GSE_GPL570, "GSE_GPL570.csv")

zscore<-function(X){(X- mean(X))/sqrt(var(X))} #standardization function
GSE_HIP<-apply(GSE_HIP, MARGIN = 2, zscore) #standardized expression data
GSE_HIP<-as.data.frame(GSE_HIP)
...
gene_name<-as.data.frame(cbind(c(1:nrow(GSE_HIP)), rownames(GSE_HIP))) #Probe Id
...
for(i in uni){
  ...
  if(is.na(info$entrez_Gene_ID[i])){ next }
    ...
    x<-apply(GSE_HIP[flag,], MARGIN = 1, IQR) #max quantiles are selected
    info[flag[which(x!=max(x))],]<-NA # rest duplicate entries are set to NA in the meta data
    GSE_HIP[flag[which(x!=max(x))],]<-NA # rest duplicate entries are set to NA in the gene expression data
  }
}
GSE_HIP[no_info,]<-NA #uniformative probes are set to NA in gene expression data
meta_data<-vec[which(!is.na(GSE_HIP[,1])),] # filtering out the NA data from the meta data
GSE<-GSE_HIP[which(!is.na(GSE_HIP[,1])), ] # filtering out the NA data from the expression data
GSE<-cbind(meta_data, GSE) #combined meta data and expression data
##write.csv(GSE, "GSE_HIP_normalized_processed.csv")
...
##write.csv(Filter_GSE, "GSE_HIP_ML.csv", row.names = F)
...
shuffle<-c(read.table("C:/Protein/AD Gene Expression/Abhibhav/Hippo/seed.txt", header = T))[[1]] #beacuse my stupid ass has forgot to put seed
